...
x_boundary = torch.rand(n_data) * (X1 - X0) + X0
y_boundary = torch.ones(n_data) * Y0
u_boundary = boundary_condition(x_boundary).unsqueeze(1)
xy_boundary = torch.stack((x_boundary, y_boundary), dim=1)

x_control = torch.rand(n_data) * (X1 - X0) + X0
y_control = torch.ones(n_data) * Y1
xy_control = torch.stack((x_control, y_control), dim=1)
x_control = x_control.unsqueeze(1)

x_periodic_boundary0 = torch.ones(n_data) * X0
y_periodic_boundary0 = torch.rand(n_data) * (Y1 - Y0) + Y0
xy_periodic_boundary0 = torch.stack((x_periodic_boundary0, y_periodic_boundary0), dim=1)

x_periodic_boundary1 = torch.ones(n_data) * X1
y_periodic_boundary1 = torch.rand(n_data) * (Y1 - Y0) + Y0
...
def closure():
    ...
    boundary_loss += criterion(u_periodic_boundary_jacobian0, u_periodic_boundary_jacobian1)
    ...
    print(f"Epoch: {epoch:5d}, B: {boundary_loss.item():.8f}, F: {physics_loss.item():.8f}, C: {cost.item():.8f}")
