...
def pinn(t, params):
    ...
    f1 = dx[:, 0] - x[:, 1]
    f2 = dx[:, 1] - (mu * (1.0 - (x[:, 0]**2.0)) * x[:, 1] - x[:, 0])
...
# x_boundary = jnp.expand_dims(x_batch[0, :], 0)
t_boundary = t_batch[::100, :]
x_boundary = x_batch[::100, :]

t_interior = t_batch[1:, :]
x_interior = x_batch[1:, :]

t_pinn = jnp.expand_dims(jnp.linspace(0.0, 20.0, 100), 1)
