...
def heat(t, x, k=1.0):
    return torch.sin(np.pi * x) * torch.exp(-((k * np.pi)**2.0) * t)
...
tx_data = torch.stack((t_data, x_data), dim=1)
u_data = heat(t_data, x_data).unsqueeze(1)
...
def closure():
    ...
    optimizer.zero_grad()
    u_pred = model(tx_data)
    loss = criterion(u_pred, u_data)

    u_t = model_jacobian(tx_data)[:, 0]
    u_hessian = model_hessian(tx_data)[:, 1:, 1:]
    u_nabla = functorch.vmap(torch.trace)(u_hessian)
    f = u_t - (k_est**2) * u_nabla
    loss += 0.01 * torch.mean(f**2)
...
with torch.no_grad():
    ...
    plt.ylabel(r"$x$")
    plt.tight_layout()
    plt.savefig("heat1d_discovery.pdf")
    plt.show()
